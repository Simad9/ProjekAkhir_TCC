...
const EditMenuPage = () => {
    ...
    const fetchMenuData = async () => {
      try {
        ...
        if (response.data) {
          ...
          setFormData({
            gambar: null,
            imagePreview: data.gambar,
            nama_menu: data.nama_menu,
            deskripsi: data.deskripsi,
            harga: data.harga.toString(),
            kategoriId_kategori: data.kategoriId_kategori.toString()
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/menu/${id}`, data, config);
        ...
        <form onSubmit={handleSubmit} className="flex gap-8">
          <div className="flex-1">
            <div className="bg-white p-6 rounded-lg shadow-lg">
              <h2 className="text-lg font-semibold mb-4">Foto Menu</h2>
